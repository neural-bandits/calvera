...
class NeuralUCB(AbstractBandit):
    def __init__(self, network: nn.Module, n_arms: int, n_features: int, 
                 lambda_: float = 1.0, nu: float = 1.0):
    ...
    def forward(self, contextualised_actions: torch.Tensor) -> torch.Tensor:
        ...
        for f_t_a in f_t_a_list:
            ...
            g_t_a = torch.cat([p.grad.flatten().detach() for p in self.theta_t.parameters()])
            g_t_a_list.append(g_t_a)
            
            # Compute uncertainty using diagonal approximation
            exploration_term = torch.sqrt(torch.sum(self.lambda_ * self.nu * g_t_a * g_t_a / self.Z_t))
