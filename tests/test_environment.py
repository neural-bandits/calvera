...
def sample_data():
    ...
    # rewards shape: (2, 3)
    rewards = torch.tensor([[0.4, 0.2, 0.9],
                            [0.1, 0.8, 0.7]])
    
    dataset = TensorDataset(contexts, rewards)
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False)
    return dataloader, contexts, rewards

def test_environment_iterator_length(sample_data):
...
def test_environment_iteration(sample_data):
    dataloader, contexts, rewards = sample_data
    env = BanditBenchmarkEnvironment(dataloader)
    ...
    for i, batch_contexts in enumerate(env):
        ...
        assert torch.allclose(env._last_contextualized_actions, batch_contexts)
        assert (previous_all_rewards is None and env._last_all_rewards is not None) or not torch.allclose(env._last_all_rewards, previous_all_rewards)
        previous_all_rewards = env._last_all_rewards

def test_get_feedback(sample_data):
    dataloader, contexts, rewards = sample_data
    env = BanditBenchmarkEnvironment(dataloader)
    ...
    assert len(feedback_dataset) == 1  # since batch_size=1
...
def test_compute_regret(sample_data):
    dataloader, contexts, rewards = sample_data
    env = BanditBenchmarkEnvironment(dataloader)
