import torch
from calvera.utils.action_shape_adaptor import ImageActionAdaptor, TextActionAdaptor
from torch import nn


def test_image_action_adaptor() -> None:
    embedding_size = 10
    batch_size = 2
    c = 3
    h = 32
    ...
    network = nn.Sequential(nn.Flatten(), nn.Linear(h * w * c, embedding_size))
...
def test_action_adaptor() -> None:
    batch_size = 2
    embedding_size = 10
    seq_length = 32
    token_embedding_size = 8

    network = nn.Sequential(nn.Flatten(), nn.Linear(seq_length * token_embedding_size, embedding_size))
